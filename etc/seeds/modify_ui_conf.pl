...
my $cmd = "ps aux | grep launch\.py ";
print "$cmd\n";
system($cmd);
print "\n";

print "Discover and Kill Running UI\n";
$cmd = "ps aux | grep launch\.py | awk '{print \$2}'";
print "$cmd\n";
...
if( $source_lst[$clc_index] eq "BZR" || $source_lst[$clc_index] eq "SRC" ){
	$ui_conf_file = "/root/euca_builder/eee/eucalyptus-ui/server/eui.ini";
}else{
